...
/* ----------  NEW COMPONENT  ---------- */
import ReportIssueForm from "./ReportIssueForm";   // <-- your form

/* --------------------------------------------------- */
const ProjectbugList = ({ projectId, teamLeadId }) => {
  const { currentUser, isTeamLead } = useCurrentUser(teamLeadId);
    const { project:{ data: projectData }, status, successMessage } = useSelector((state) => state.project);
  ...
  const employeeId = currentUser?.id;
  const employeeProjectBugs = useSelector((state) =>
    selectEmployeeProjectBugs(state, projectId, employeeId)
  );
...
console.log(projectData);
  ...
  const bugs = useMemo(() => {
    if (isCpc || (isTeamLead && viewMode === "all")) return bugsByProjectId || [];
    if (viewMode === "my") return employeeProjectBugs || [];
    return bugsByProjectId || [];
  }, [isCpc, isTeamLead, viewMode, bugsByProjectId, employeeProjectBugs]);
  ...
  const handleDownloadReport = async () => {
    try {
      if (assignedToFilter !== "all") {
        const memberId = assignedMembersMap[assignedToFilter];
        await dispatch(downloadBugsByMemberId({ projectId, memberId })).unwrap();
      } else {
        await dispatch(downloadBugsByProjectId(projectId)).unwrap();
      }
    ...
    <TooltipProvider>
      <div className="w-full bg-white">
        ...
        <div className="flex flex-wrap gap-3 p-4">
          ...
          {showAllViewOption && (
            <Button className="bg-blue-600" variant="default" onClick={handleDownloadReport}>
          ...
          <Table>
            <TableHeader>
              <TableRow className="bg-gray-50">
                ...
                <TableHead onClick={() => handleSort("assignedToDetails.memberName")} className="cursor-pointer">
                  Assignee
                </TableHead>
            ...
            <TableBody>
              {paginatedBugs.length === 0 ? (
                <TableRow>
                  <TableCell colSpan={showActionColumn ? 7 : 6} className="text-center py-8 text-gray-500">
                    No bugs found
                  </TableCell>
                </TableRow>
              ) : (
                paginatedBugs.map((bug, idx) => (
                  <TableRow
                    ...
                    onClick={() => handleViewBug(bug)}
                  >
                    <TableCell>{(currentPage - 1) * bugsPerPage + idx + 1}</TableCell>
                    <TableCell>{truncate(bug.title)}</TableCell>
                    <TableCell>
                      <Tooltip>
                        <TooltipTrigger>
                        ...
                        <TooltipContent>{bug?.assignedToDetails?.memberName || "Unassigned"}</TooltipContent>
                      </Tooltip>
                    </TableCell>
                    <TableCell>
                      <span
                    ...
                    <TableCell>{formatDateTimeIST(bug.deadline) || "—"}</TableCell>
                    <TableCell>
                      <span
                        className={cn(
                          "px-2 py-1 rounded-full text-xs font-medium",
                          ...
                          bug.status === "Completed" && "bg-green-100 text-green-800"
                        )}
                      >
                        {bug.status}
                      </span>
        ...
        {totalPages > 1 && (
          <div className="flex items-center justify-between py-3 px-4">
            <span className="text-sm text-gray-600">
              Showing {(currentPage - 1) * bugsPerPage + 1}-
              {Math.min(currentPage * bugsPerPage, filteredBugs.length)} of {filteredBugs.length}
            </span>
        ...
        <Dialog open={showFilterDialog} onOpenChange={setShowFilterDialog}>
          <DialogContent className="max-w-md">
            ...
            {/* … same filter UI you already had … */}
            <div className="space-y-4">
              {/* priority, status, assignee, dates … */}
              {/* (copy-paste your existing filter UI here) */}
