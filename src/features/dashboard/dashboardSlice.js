...
export const fetchTasksByDeadline = createAsyncThunk(
  ...
  async () => {
    try {
      const response = await axiosInstance.get('/task/bydeadline');
      console.log('fetchTasksByDeadline response:', response.data);
      // Assuming response.data.data is an array of tasks sorted by deadline
      const tasks = response.data.data || [];
      // Sort tasks by deadline (ascending) for consistent display
